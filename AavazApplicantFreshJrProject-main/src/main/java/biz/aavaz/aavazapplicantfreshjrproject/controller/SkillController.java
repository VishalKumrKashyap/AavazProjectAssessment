...
public class SkillController {
    ...
    public void addSkill(String path) throws FileNotFoundException {
        ...
        // validating skill is already exit in system or not
        
        if(skillDao.validate(skill)==1) 
        {
        skillDao.save(skill);
        }
        else if (skillDao.validate(skill)==-1)
        {
        System.out.println(skill.getId() + " is already present in system");
        }
    }
